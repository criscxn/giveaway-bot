...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    const data = db.get(`cekilis_${key}`);
    const sonaerdimi = db.fetch(`son_${key}`);
    if (!sonaerdimi)
      return interaction.reply(
        "Bu çekiliş henüz sona ermemiş veya böyle bir çekiliş yok!"
      );
    ...
    interaction.reply({
      content: "Başarıyla çekiliş yeniden çekildi.",
      ephemeral: true,
    });
    db.delete(`cekilis_${interaction.channel.id}`);
    interaction.channel.send(
      "<@" + kazanan + "> Tebrikler **" + data + "** Kazandın!"
    );
  },
};
