...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    const mesajs = data.mesajid;
    const mesaj = await interaction.channel.messages.fetch(mesajs);
    const kullanici = db.fetch(`user_${key}`);
    if (!kullanici) return interaction.reply("Yeterli katılımcı bulunamadı.");
    const kazanan = kullanici[Math.floor(Math.random() * kullanici.length)];
    const katılımcı = db.get(`user_${key}`).length;
    ...
    interaction.reply({
      content: "Başarıyla çekiliş bitirildi.",
      ephemeral: true,
    });
    ...
    interaction.channel.send(
      "<@" + kazanan + "> Tebrikler **" + data.odul + "** Kazandın!"
    );
  },
};
